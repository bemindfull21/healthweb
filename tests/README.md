...
| `test_sale.py` | ERP 재고 조회(입고 전 미노출, 남은수량/단가/남은금액), 재고 초과 판매 거부, 부분/전량 판매+재고 재계산, 판매 목록, 판매 수정(PATCH)+초과 변경 거부, 타 사용자 수정 불가(404), 판매 삭제(재고 복원), 재고 폐기(비용 자동 생성)+수정 거부+삭제 시 비용도 삭제, 판매 이력 있는 구매 삭제 거부(400) |
