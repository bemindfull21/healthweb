...
| `test_erp.py` | ERP 권한 게이팅, 오너 부여/회수, 영수증 업로드+Gemini 추출(실제 API 호출, 한국어 번역+수량+상품 사진 썸네일 크롭+금액 총액/단가 계산), 저장(구매ID 채번)/목록/합계/삭제+미디어 GC, 입고 체크(PATCH)+미입고만 조회, 구매일자 기간 필터, 수량 변경(PATCH)+단가 재계산+1 미만 거부 |
| `test_expense.py` | ERP 비용 권한 게이팅, 등록(구매ID 선택)/빈 값 거부/존재하지 않거나 타 사용자 구매ID 거부/구매ID 미지정("기타") 등록, 기간 조회+합계+구매ID 표시, 금액 수정(PATCH)/존재하지 않는 기록 404, 타 사용자 수정·삭제 불가(404), 삭제 |
| `test_sale.py` | ERP 재고 조회(입고 전 미노출, 남은수량/단가/남은금액/썸네일), 게시 여부(`is_listed`) 기본값·PATCH 토글·`unlisted_only` 필터, 재고 초과 판매 거부, 부분/전량 판매+재고 재계산, 판매 목록(썸네일 포함), 판매 수정(PATCH)+초과 변경 거부, 타 사용자 수정 불가(404), 판매 삭제(재고 복원), 재고 폐기(비용 자동 생성+구매ID 연결)+수정 거부+삭제 시 비용도 삭제, 판매 이력 있는 구매 삭제 거부(400), 폐기 비용을 비용 탭에서 직접 삭제·금액수정 시도하면 거부(400), 판매 목록 폐기 건 금액이 수량×구매단가로 표시(waste_value_krw), 이미 판매·폐기된 수량보다 적게 구매 수량 변경 불가 |
