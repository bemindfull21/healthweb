...
| GET | `/sales?date_from=&date_to=` | ✓(erp_access) | — | `sale_date` 기준 기간 필터(생략 시 전체). `purchase_item` 조인으로 `purchase_no`·`product_name`·`thumb_url` 포함. 각 item에 `is_waste`(bool). `{items:[...], total_krw}` |
