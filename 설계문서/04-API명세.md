...
| PATCH | `/purchases/:id` | ✓(erp_access) | `received?: bool, quantity?: int, is_listed?: bool` | 셋 다 선택, 보낸 필드만 수정. `received`는 입고 체크/해제. `quantity`는 1 이상이어야 하고(아니면 400) 이미 판매·폐기된 수량보다 적게 줄이면 400 "이미 판매·폐기된 수량보다 적게 설정할 수 없습니다" — 통과하면 `unit_price_krw`도 `price_krw/quantity`로 재계산(2026-09-13 추가). `is_listed`는 판매사이트 게시 여부(2026-09-15 추가). 본인 기록만(아니면 404) |
...
| GET | `/stock?unlisted_only=` | ✓(erp_access) | — | `received=1`이고 `quantity - sum(sale_qty) > 0`인 구매 건 목록(남은 재고는 저장 컬럼이 아니라 매번 `sale_item` 집계로 계산). `unlisted_only=true`면 `is_listed=0`(미게시)인 건만. 각 item: `purchase_item_id`·`purchase_no`·`product_name`·`remaining_qty`·`unit_price_krw`·`remaining_amount_krw`(=단가×남은수량)·`is_listed`(bool, 판매사이트 게시 여부, 2026-09-15 추가)·`thumb_url` |
