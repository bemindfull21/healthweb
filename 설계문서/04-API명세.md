...
| PATCH | `/purchases/:id` | ✓(erp_access) | `received?: bool, quantity?: int` | 둘 다 선택, 보낸 필드만 수정. `received`는 입고 체크/해제. `quantity`는 1 이상이어야 하고(아니면 400) 이미 판매·폐기된 수량보다 적게 줄이면 400 "이미 판매·폐기된 수량보다 적게 설정할 수 없습니다" — 통과하면 `unit_price_krw`도 `price_krw/quantity`로 재계산(2026-09-13 추가). 본인 기록만(아니면 404) |
