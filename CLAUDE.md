...
- **캐시버스터**: `app.js`·`style.css`·`config.js`·`landing.js`도 아이콘과 동일하게 참조하는 HTML(`app.html`·`index.html`·`challenge.html`·
  `about.html`·`install-guide.html`)에서 `?v=N`로 부른다(2026-09-12 도입, 파일별로 따로 버전 관리 — 현재 `app.js`는 `v=11`,
  `style.css`는 `v=9`, `config.js`/`landing.js`는 `v=1`). Fastly CDN이 `max-age=600`이라 버전을 안 올리면
...
채움(기존 `GET /purchases`가 이미 `id desc`로 정렬해 주므로 별도 정렬 로직 불필요) — 선택은 선택사항, 안 하면 "기타"로 저장.

**폐기 자동생성 비용은 금액도 잠금**(2026-09-13): `PATCH /expenses/:id`도 `DELETE`와 같은 패턴으로 `sale_item.expense_item_id`
참조 여부를 먼저 확인해, 폐기 판매가 만든 비용이면 400 "폐기로 자동 생성된 비용은 금액을 변경할 수 없습니다"로 막음 —
폐기 비용은 `수량×구매단가`로 이미 정확히 계산된 값이라 손으로 바꾸면 실제 폐기 수량/비용과 어긋나기 때문. 고치려면
판매 목록에서 그 폐기 건을 지우고 다시 등록해야 함(수량을 바꿔서).
...
**레이아웃도 판매 탭과 동일하게 2단 개편**(2026-09-13) — 1단(체크박스·사진·상품명 `trunc()` 10자 말줄임·구매ID/재고/단가),
2단(체크 시 펼쳐지는 수량·판매가 입력칸 + 남은금액). 상품명이 길어 남은금액이 찌그러지던 문제를 판매 탭과 같은 방식으로 해결.
...
**레이아웃 개선**(2026-09-13): 상품명이 길면 좁은 화면에서 여러 줄로 밀려 수량/가격 입력칸과 판매금액이 다 찌그러지는
문제가 있어(실사용자 스크린샷으로 확인) 행을 2단으로 재배치 — 1단(`.erp-item-top`)은 사진·상품명(`trunc()`로 10자+"…"
말줄임, `title` 속성으로 전체 이름은 hover/길게 눌러 확인 가능)·구매ID·삭제 버튼, 2단(`.erp-item-bottom`)은 수량/가격
입력칸과 판매금액을 한 줄에 배치해 금액이 항상 넉넉한 공간을 갖도록 함. `trunc()`는 `app.js` 상단 유틸에 추가한 공용 헬퍼.
